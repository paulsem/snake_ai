...
class BestAI(object):
    ...
    def makeReward(self, game):
        ...
        if game.game_over():
            ...
            self.final_reward = -math.sqrt(
                (game.player.head.pos.x - game.food.pos.x) ** 2 + (game.player.head.pos.y - game.food.pos.y) ** 2)/100
    ...
    def train_short_memory(self, state, action, reward, next_state, over):
        target = reward
        if not over:
            target = reward + self.y * np.amax(self.model.predict(next_state.reshape((1, self.input)))[0])
        target_f = self.model.predict(state.reshape((1, self.input)))
        target_f[0][np.argmax(action)] = target
        self.model.fit(state.reshape((1, self.input)), target_f, epochs=1, verbose=0)
